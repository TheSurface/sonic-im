...
if sonic_im_client == 'Keeps':
    ...
    ### VIEWS: Performance Summary, Chartable vs. Looker, Chartable-Looker Combined by Show ###
        ...
        # Aggregate purchase and lead data by date and show name
        daily_budget_df['Percent Male'] = daily_budget_df['% M/F'].apply(lambda y: int(y.split('/')[0].split(' ')[1])/100)
        daily_budget_df['Percent Female'] = daily_budget_df['% M/F'].apply(lambda y: int(y.split('/')[1].split(' ')[2])/100)
