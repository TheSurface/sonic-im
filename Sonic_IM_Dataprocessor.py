...
if sonic_im_client == 'Keeps':
    ...
    ### VIEWS: Monthly Calendar View ###
        ...
        df_budget_grouped = df_budget[df_budget['Broadcast Week'] <= cutoff_date].groupby(['Show Name','Actual Drop Day']).sum()[['Client Rate']].reset_index()
        ...
        # Combine base and budget dataframes
        df_base_budget = pd.merge(df_base, df_budget_grouped, how='left', left_on=['date','Show Name'], right_on=['Actual Drop Day','Show Name'])
        df_base_budget['Client Rate'].fillna(0,inplace=True)



        # Create lead and purchase calendar dataframes
        df_final_output = pd.merge(df_base_budget, df_budget[['event_date','created_week','created_month','Show Name']],how='left',left_on=['date','Show Name'],right_on=['created_month','Show Name'])
