...
if sonic_im_client == 'Keeps':
    ...
    if (uploaded_daily_budget is not None) and (uploaded_chartable_data is not None):
        ...
        WHERE 
            (a."Broadcast Week" <= "{cutoff_date}" AND ((b.Date >= a."Broadcast Week" AND b.Date < a.next_drop_date) OR
            (a."Broadcast Week" = a.next_drop_date AND b.Date >= a.next_drop_date))) OR
            (a."Broadcast Week" <= "{cutoff_date}" AND b.Date IS NULL) OR
            (a."Broadcast Week" <= "{cutoff_date}" AND ((b.Date >= a."Broadcast Week" AND b.Date >= a.next_drop_date) OR 
            (b.Date <= a."Broadcast Week" AND b.Date <= a.next_drop_date)))
